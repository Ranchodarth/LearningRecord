...
import { getToken, removeToken, setToken, setUserName, setavatarUrl } from "@/utils/cache/cookies"
...
export const useUserStore = defineStore("user", () => {
  ...
  const login = (loginData: ILoginRequestData) => {
    return new Promise((resolve, reject) => {
      loginApi({
        ...
        .then((res) => {
          console.log("wokankan", res.data)
          setToken(res.data.token)
          token.value = res.data.token
          nickName.value = res.data.userInfo.nickName
          setUserName(res.data.userInfo.nickName)
          setavatarUrl(res.data.userInfo.avatarUrl)
          resolve(true)
        })
        .catch((error) => {
          reject(error)
  ...
  return { token, roles, loginName, nickName, setRoles, login, getInfo, changeRoles, logout, resetToken }
})
